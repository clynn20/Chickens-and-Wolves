...
bool is_the_same(State s, State n){
...
bool is_visit(vector<Node*> v_nodes, State s){
    vector<Node*>::iterator itr;
    for(itr=v_nodes.begin(); itr!=v_nodes.end();itr++){
        if(is_the_same((*itr)->state,s)){
            return true;
        }
    }
    return false;
...
Node* bfs(State init,State goal, int& expand){
    ...
    while(!q.empty()){
        ...
        q.pop();
        // check if it is goal 
        if(is_the_same(node->state,goal)){
            return node;
        }
        expand++;
        ...
        State newstate;
        // move one chicken
        if(succ(node->state,&newstate,1,0) && is_valid(newstate) && !is_visit(v_nodes,newstate)){
            bfs_add_ntoq(node,newnode,newstate,v_nodes,q);
        }
        // move two chicken
        if(succ(node->state,&newstate,2,0) && is_valid(newstate) && !is_visit(v_nodes,newstate)){
            bfs_add_ntoq(node,newnode,newstate,v_nodes,q);
        }
        // move one wolf
        if(succ(node->state,&newstate,0,1) && is_valid(newstate) && !is_visit(v_nodes,newstate)){
            bfs_add_ntoq(node,newnode,newstate,v_nodes,q);
        }
        // move one chicken one wolf
        if(succ(node->state,&newstate,1,1) && is_valid(newstate) && !is_visit(v_nodes,newstate)){
            bfs_add_ntoq(node,newnode,newstate,v_nodes,q);
        }
        // move two wolves
        if(succ(node->state,&newstate,0,2) && is_valid(newstate) && !is_visit(v_nodes,newstate)){
            bfs_add_ntoq(node,newnode,newstate,v_nodes,q);
        }
    }
    // no solution found 
...
Node* iddfs(State init,State goal, int& expand){
    ...
    // vector to store the visited nodes
    vector<Node*> v_nodes;
    // the depth that increase in each iteration 
    int it_depth = 0;
    // the limit max depth
    int max_depth = 600;
    bool found = false;
    ...
    while(1){
        ...
        while(!st.empty()){
            ...
            expand++;
        // add successor node only if the node depth didn't exceed the current iteration depth limit
        ...
        // increase the depth after each iteration
        it_depth++;
...
void print_path(Node* solution,int& expand,char* out_fname){
        int count = 0;
        State temp;
        stack<State> stk; 
        ofstream out_f;
...
int main(int argc, char** argv){
        ...
        if(strcmp(argv[3],"bfs")==0){
            solution = bfs(init,goal,expand);
        }
        if(strcmp(argv[3],"dfs")==0){
            solution = dfs(init,goal,expand);
        }
        if(strcmp(argv[3],"iddfs")==0){
            solution = iddfs(init,goal,expand);
        }
        if(strcmp(argv[3],"astar")==0){
            solution = astar(init,goal,expand);
        }
        print_path(solution,expand,argv[4]);
